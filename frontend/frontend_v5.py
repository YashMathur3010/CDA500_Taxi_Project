...
def create_taxi_map(shapefile_path, prediction_data):
    ...
    # Merge with cleaned column names. Use a left merge to keep all zones
    ...
    return nyc_zones  # Return the merged GeoDataFrame
...
with st.spinner(text="Plot predicted rides demand"):
    ...
    zones_with_predictions = create_taxi_map(shapefile_path, predictions)
    ...
    # Merge predictions with zones_with_predictions to get zone names
    predictions_with_names = predictions.merge(
        zones_with_predictions[['pickup_location_id', 'zone']],
        on='pickup_location_id',
        how='left'
    )

    # Show sample of the data with zone names
    st.dataframe(
        predictions_with_names[['zone', 'pickup_location_id', 'predicted_demand']]
        .sort_values("predicted_demand", ascending=False)
        .head(10)
        .reset_index(drop=True)
    )

    # Add dropdown menu to select location
    st.subheader("Select Location")
    location_names = zones_with_predictions['zone'].unique()
    selected_location = st.selectbox("Choose a location:", location_names)

    # Get the corresponding location_id for the selected location name
    selected_location_id = zones_with_predictions[zones_with_predictions['zone'] == selected_location]['pickup_location_id'].iloc[0]

    # Filter features and predictions based on selected location ID
